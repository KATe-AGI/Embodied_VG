...
此前 `z_approach` 实际表示可见表面朝相机一侧的法向。该方向适合用于从表面点沿 `-z_approach` 偏移到物体内部，但如果直接把输出姿态作为机械臂末端姿态使用，会让机械臂的 `+Z` 接近方向与实际接近方向相反。

### 修改

- `plug_vg/geometry.py` 中的抓取帧定义更新为：
  - `+X` 仍然保持 `tail -> head`。
  - `+Z` 改为从相机可见侧指向物体内部的接近方向。
  - `Y` 随右手系同步翻转，保持旋转矩阵合法。
- 抓取中心补偿由沿 `-z_approach` 偏移改为沿 `+z_approach` 偏移半个抓取区域厚度。
- JSON 中现有字段名保持不变，但 `axes.z_approach` 现在表示真正的接近方向。
